...
$name=$_POST['route_id'];  
$location=$_POST['date'];  
$query = "insert into stock_unload_receive(route_id,date) VALUES ('$name','$location')";  
$db->query($query);
$id=$_POST['route_id']; 
foreach ($_POST['productid'] as $i => $value) {
    $query_r = "INSERT INTO stock_unload SET date = '{$location}',product_id = '{$_POST['productid'][$i]}',quantity = '{$_POST['quantity'][$i]}', received_return = '{$_POST['received_return'][$i]}'";
    $db->query($query_r);
    }
}
?>
