    ...
    $( document ).ready(function() {
      ...
      $('#assign_button').click(function() {
        ...
        var bool = document.getElementById("user_p").value;
          if (password == <?php echo $password['password'];?>){
              window.location = "stock_unload_manage_edit.php?id=" +bool;
          } else {
      ...
      $('#delete_all_button').click(function() {
        ...
        var bool = document.getElementById("user_delete").value;
          if (password == <?php echo $password['password'];?>){
             delete_bill(bool, "stock_unload_receive");
          } else {
             swal("Error!", "Password you entered is wrong!", "warning");
          }
      ...
      $('#add_button').click(function() {
        ...
        var bool = document.getElementById("user_add").value;
          if (password == <?php echo $password['password'];?>){
              window.location = "stock_unload_manage_edit_add.php?id=" + bool;
          } else {
              swal("Error!", "Password you entered is wrong!", "warning");
          }
        $('#addModal').modal('hide');
...
if(isset($_GET['ok'])){
  ...
  $received_return = $_GET['received_return'];
  $query_r = "update stock_unload set product_id='{$productname}', quantity='{$quantity}', received_return = '{$received_return}' where id = '{$id_update}'";
  $result_r = $db->query($query_r);
}
if(isset($_POST['save_add']))  
{ 
$query_t = "UPDATE stock_unload_receive SET route_id = '{$_POST['route_id']}' WHERE date = '{$_GET['date_add']}'";
$db->query($query_t);

if($_POST['productid']==''){
  $session->msg('s',"no items to add but updated ");
  redirect('stock_unload_manage.php', false);
}else{
  $query_r = "INSERT INTO stock_unload SET date = '{$_GET['date_add']}',product_id = '{$_POST['productid']}',quantity = '{$_POST['quantity']}',received_return = '{$_POST['received_return']}'";
  $db->query($query_r);
}
} 
?>
  <div class="panel">
        ...
        <div class="panel-heading clearfix">
             ...
             <div class="col-xs-5">
                <a href="stock_unload.php" class="btn btn-primary">Add New</a>
              </div>
            </div>
          </div>
      </br>
                ...
                <td class="text-center">
                  ...
                  <thead>
                    <tr>
                      ...
                      <th class="text-center" > Reveived Return </th>
